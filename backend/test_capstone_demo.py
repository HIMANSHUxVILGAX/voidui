...
"""
================================================================================
🐝 NO-ASH Module 4 — elumPot Deception System
...
def run_capstone_demo():
    print("#" * 70)
    print(" 🐝 STARTING ELUMPOT CAPSTONE END-TO-END DEMO TEST SUITE")
    print("#" * 70)
    ...
    print_header("2. REST API Status & Configuration Handler (/api/honeypot/config)")
    status = honeypot_service.get_status()
    print_result("2.1", "Get Status Endpoint", status.get("daemon_active", False), f"SSH: {status.get('ssh_port')}, Web: {status.get('web_port')}")

    config_res = honeypot_service.update_config(2222, 8080)
    print_result("2.2", "POST /api/honeypot/config Handler", config_res.get("status") == "success", f"Config Response: {config_res.get('message')}")
    ...
    try:
        ...
        threading.Timer(0.3, lambda: honeypot_service.allow_ip_proceed("127.0.0.1")).start()
        ...
        print_result("3.1", "SSH Socket Intercept & Mirror Shell", has_ssh_logs, f"SSH Banner: '{banner.strip()}' | Logs Captured: {len(ssh_logs.get('logs', []))}")
    except Exception as e:
        print_result("3.1", "SSH Socket Intercept & Mirror Shell", False, str(e))
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (CapstoneDemo/1.0)"})
        resp = urllib.request.urlopen(req, timeout=5)
        body = resp.read().decode("utf-8", errors="ignore")

        decoys = honeypot_service.get_ai_decoys()
        has_decoys = len(decoys.get("decoys", [])) > 0
        print_result("4.1", "Web Decoy Intercept & AI Synthetic Vault", resp.status == 200 and has_decoys, f"HTTP Code: {resp.status} | Decoys Served: {len(decoys.get('decoys', []))}")
    except Exception as e:
        print_result("4.1", "Web Decoy Intercept & AI Synthetic Vault", False, str(e))

    # 5. Live Attacker Keystroke Streaming Endpoint Verification
    print_header("5. Live Keystroke Stream Endpoint (/api/honeypot/keystrokes)")
    keystrokes_res = honeypot_service.get_active_sessions_keystrokes()
    total_sessions = keystrokes_res.get("total_sessions", 0)
    print_result("5.1", "Keystroke Stream Endpoint", total_sessions > 0, f"Active Sessions Tracked: {total_sessions}")
    ...
    print_result("6.1", "Attack Scenario Simulator", sim_res.get("status") == "success", f"Simulated IP: {sim_res.get('simulated_log', {}).get('attacker_ip')}")

    # 7. Clean Daemon Teardown
    print_header("7. Daemon Clean Teardown")
    res_stop = honeypot_service.stop_daemon()
    print_result("7.1", "Daemon Clean Stop", not honeypot_service.is_running, f"Message: {res_stop.get('message')}")

    print("\n" + "=" * 70)
    print(" 🎉 ALL ELUMPOT CAPSTONE VERIFICATION CHECKS COMPLETED SUCCESSFULLY!")
    print("=" * 70 + "\n")
