...
def run_all_tests():
    print("\n" + "#" * 60)
    print("🚀 STARTING OPTICS PHASE 1 AUTOMATED TEST SUITE")
    ...
    try:
        ...
        print_result("1.1", "Singleton Check", is_singleton,
                     f"Same instance ID: {id(s1)}")
    ...
    try:
        ...
        print_result("1.2", "Initial State Verification", init_ok,
                     f"Ports: SSH={honeypot_service.ssh_port}, Web={honeypot_service.web_port}, Running={honeypot_service.is_running}")
    except Exception as e:
    ...
    try:
        ...
        passed = (res.get("status") ==
                  "success" and honeypot_service.is_running is True)
    ...
    try:
        ...
        print_result("2.2", "Already Running Guard",
                     passed, f"Response: {res}")
    except Exception as e:
    ...
    try:
        ...
        print_result("2.3", "Port Conflict Error Catch", passed,
                     f"Captured Error Message: '{res.get('message')}'")
    except Exception as e:
    ...
    try:
        ...
        passed = (res.get("status") ==
                  "success" and honeypot_service.is_running is False)
    ...
    if not honeypot_service.is_running:
        honeypot_service.start_daemon()
    print_header(
        "3.1", f"SSH Socket Connection & Log Capture (Port {honeypot_service.ssh_port})")
    try:
        import threading
        threading.Timer(
            0.2, lambda: honeypot_service.allow_ip_proceed("127.0.0.1")).start()
        ...
        print_result("3.1", "SSH Socket Test", has_ssh_log,
                     f"SSH Banner Received: '{banner.strip()}', Total SSH Logs: {len(honeypot_service.ssh_logs)}")
    except Exception as e:
        print_result("3.1", "SSH Socket Test", False, str(e))

    # --- 3.2 Web Decoy Connection ---
    print_header(
        "3.2", f"Web Decoy HTTP Connection & Trace Capture (Port {honeypot_service.web_port})")
    try:
        import threading
        threading.Timer(
            0.5, lambda: honeypot_service.allow_ip_proceed("127.0.0.1")).start()
        url = f"http://127.0.0.1:{honeypot_service.web_port}/admin/confidential_passwords.txt"
        req = urllib.request.Request(
            url, headers={"User-Agent": "AttackerBrowser/1.0"})
        resp = urllib.request.urlopen(req, timeout=5)
        body = resp.read().decode("utf-8", errors="ignore")

        has_web_log = len(honeypot_service.web_traces) > 0 or len(
            honeypot_service.web_logs) > 0
        print_result("3.2", "Web Decoy Test", has_web_log,
                     f"HTTP Status: {resp.status}, Total Web Traces: {len(honeypot_service.web_traces)}")
    except Exception as e:
    ...
    try:
        ...
        passed = ("192.168.1.50" in honeypot_service.blocked_ips and honeypot_service.decision_results.get(
            "192.168.1.50") == "block")
        print_result("3.3", "SECURE & BLOCK Decision",
                     passed, f"Response: {res}")
    except Exception as e:
    ...
    try:
        ...
        passed = (honeypot_service.decision_results.get(
            "192.168.1.51") == "view")
        print_result("3.4", "VIEW & MANIPULATE Decision",
                     passed, f"Response: {res}")
    except Exception as e:
    ...
    try:
        ...
        print_result("4.1", "IP Geolocation Resolver", passed,
                     f"Local Geo: {geo_local['location']}, Public Geo: {geo_public['location']}")
    except Exception as e:
    ...
    try:
        ...
        if len(honeypot_service.ssh_logs) > 0:
            ...
            summary_ok = "plain_english_summary" in log and len(
                log["plain_english_summary"]) > 0
        print_result("4.2", "Plain-English Log Summary", summary_ok,
                     f"Sample Summary: '{honeypot_service.ssh_logs[0].get('plain_english_summary') if honeypot_service.ssh_logs else 'N/A'}'")
    except Exception as e:
    ...
    try:
        content = honeypot_service.generate_ai_decoy_content(
            "config/passwords.txt", "10.0.0.99")
        passed = len(content) > 0
        print_result("5.1", "AI Decoy Generation", passed,
                     f"Generated Decoy Snippet: '{content[:60]}...'")
    except Exception as e:
    ...
    try:
        content2 = honeypot_service.generate_ai_decoy_content(
            "config/passwords.txt", "10.0.0.99")
        session = honeypot_service.sessions.get("10.0.0.99")
        passed = (
            session is not None and "config/passwords.txt" in session.cached_ai_decoys)
        print_result("5.2", "Session Memory Cache", passed,
                     f"Cached match: {session.cached_ai_decoys.get('config/passwords.txt') == content2}")
    except Exception as e:
    ...
    try:
        ...
        passed = (res.get("status") == "success" and len(
            res.get("decoys", [])) > 0)
        print_result("5.3", "AI Decoy Vault Retrieval", passed,
                     f"Total Cached Decoys Found: {len(res.get('decoys', []))}")
    except Exception as e:
    ...
    try:
        ...
        print_result("6.1", "Clear Logs Functionality",
                     passed, f"Response: {res}")
    except Exception as e:
    ...
    print_header(
        "6.2", "1-Click Simulated Attack Generator (simulate_attack_scenario)")
    try:
        res = honeypot_service.simulate_attack_scenario()
        passed = (res.get("status") == "success" and "simulated_log" in res)
        print_result("6.2", "Simulated Attack Generator", passed,
                     f"Simulated Attacker IP: {res.get('simulated_log', {}).get('attacker_ip')}")
    except Exception as e:
